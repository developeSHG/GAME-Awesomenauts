...
IMPLEMENT_DYNAMIC(CPathExtractor, CDialog)

CPathExtractor::CPathExtractor(CWnd* pParent /*=NULL*/)
	: CDialog(IDD_PATHEXTRACTOR, pParent)
{

}

CPathExtractor::~CPathExtractor()
{
	for_each(m_PathInfoLst.begin(), m_PathInfoLst.end(), SafeDelete<PATH_INFO*>);
	m_PathInfoLst.clear();
}

void CPathExtractor::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CPathExtractor, CDialog)
	ON_WM_DROPFILES()
	ON_BN_CLICKED(IDC_BUTTON1, &CPathExtractor::OnBnClickedSave)
	ON_BN_CLICKED(IDC_BUTTON6, &CPathExtractor::OnBnClickedLoad)
END_MESSAGE_MAP()


// CPathExtractor 메시지 처리기입니다.


void CPathExtractor::OnDropFiles(HDROP hDropInfo)
{
...
void CPathExtractor::CreateHorizontalScroll()
{
	CString strName = L"";
	CSize size;
	int iCX = 0;
...
void CPathExtractor::OnBnClickedSave()
{
...
void CPathExtractor::OnBnClickedLoad()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CFileDialog Dlg(
